...
// DMA SUPPORT Macro (ESP-IDF spezifisch)
#if defined(ESP32)
  #define LSM6DSO_DMA_ENABLED 1
#else
  #define LSM6DSO_DMA_ENABLED 0
#endif
...
public:
    ...
    // Portiert: ESP-IDF i2c_port_t anstelle von TwoWire&
    status_t beginCore(uint8_t deviceAddress, i2c_port_t i2cPort);

    // Portiert: spi_device_handle_t anstelle von SPIClass&
    status_t beginSPICore(uint8_t csPin, uint32_t spiPortSpeed, spi_device_handle_t spiDevice);

    status_t fifoburstRead(uint8_t *buffer, uint16_t length);

    status_t readMultipleRegisters(uint8_t* outputPointer, uint8_t address, uint8_t numBytes);
    status_t readRegister(uint8_t* outputPointer, uint8_t address);
    status_t readRegisterInt16(int16_t* outputPointer, uint8_t address);
    status_t writeRegister(uint8_t address, uint8_t dataToWrite);
    status_t writeMultipleRegisters(uint8_t* inputPointer, uint8_t address, uint8_t numBytes);
    status_t enableEmbeddedFunctions(bool enable = true);

#if LSM6DSO_DMA_ENABLED
    status_t readRegisterDMA(uint8_t* outputPointer, uint8_t address);
    status_t readMultipleRegisterDMA(uint8_t address, uint8_t* buffer, uint16_t length);
    status_t writeRegisterDMA(uint8_t address, uint8_t dataToWrite);
    status_t writeMultipleRegistersDMA(uint8_t inputPointer[], uint8_t address, uint8_t numBytes);
#endif

private:
#if LSM6DSO_DMA_ENABLED
    spi_device_handle_t spiDeviceHandle = nullptr;  // SPI Handle für ESP32 DMA
    bool dmaInitialized = false;                     // DMA Init Flag
#endif

	uint8_t* _fifoBurstTxBuffer = nullptr;
	uint8_t* _fifoBurstRxBuffer = nullptr;
	uint16_t _fifoBurstBufferCapacity = 0;

    uint8_t commInterface;  // I2C_MODE oder SPI_MODE
    uint8_t I2CAddress;     // I2C Adresse
    uint8_t chipSelectPin;  // GPIO Pin für SPI-CS

    i2c_port_t _i2cPort;              // ESP-IDF I2C Port (z.B. I2C_NUM_0)
    spi_device_handle_t _spiDevice;  // SPI Device Handle für SPI-Transaktionen
};

#endif // __LSM6DSOIMU_H__
...
public:
	...
	uint8_t fifoDecimation;
	uint8_t frq_fine;

	// FREQUENCY SETTINGS
	float LSBSTEP;
	uint8_t accelScale; // accel scale value for the IMU, will be set in beginSettings()
	uint8_t gyroScale;  // gyro scale value for the IMU, will be set in beginSettings()
};
...
struct fifoBytes{ 
public:
  uint8_t FIFO_DATA_OUT_TAG;
  uint8_t FIFO_DATA_OUT_X_L;
  uint8_t FIFO_DATA_OUT_Y_L;
  uint8_t FIFO_DATA_OUT_Z_L;
  uint8_t FIFO_DATA_OUT_X_H;
  uint8_t FIFO_DATA_OUT_Y_H;		
  uint8_t FIFO_DATA_OUT_Z_H;
};

// Beste Performance (20 Bytes)
#pragma pack(push, 4)
struct FifoSample {
    float timestamp;  // 0
    uint8_t id;       // 4
    uint8_t _unused[3]; // 5-7 (Padding)
    float value1;    // 8  (aligniert)
    float value2;    // 12 (aligniert)
    float value3;    // 16 (aligniert)
}; // 20 Bytes
#pragma pack(pop)

//static_assert(sizeof(FifoSample) == 15, "Invalid FifoSample size");
...
class LSM6DSO : public LSM6DSOCore
{
  public:
	...
	uint16_t decodeFifoSamples(const uint8_t* buffer, uint16_t len, std::function<void(const FifoSample&)> cb);
	FifoSample decodeFifoSample(const uint8_t* fifo_data);
    //Error checking
    uint16_t allOnesCounter;
    uint16_t nonSuccessCounter;

    LSM6DSO();
    bool begin(uint8_t address, i2c_port_t i2cPort);
    bool beginSPI(uint8_t csPin, uint32_t spiPortSpeed, spi_device_handle_t spiDevice);
    ...
    bool setTSdecimation(uint8_t mode);
    status_t getTSdecimation(uint8_t *factor);
    bool setTempSamplingRate(uint8_t rate);
    uint8_t getFifoMode();
    ...
    void fifoClear();
    fifoData fifoRead();
	FifoSample fifoRead2();
    uint16_t getFifoStatus();
	uint16_t getFifoStatus2();
    void fifoEnd();
    
    float calcGyro( int16_t );
    float calcAccel( int16_t );

    float calcAccelFifo(int16_t input);
	bool getAccelScale();
